...
describe('Pruebas de interfaz gráfica WikiGroup - Home, Actividades y Empresa', function () {
  ...
  it('Home: debería mostrar el hero con el título WikiGroup y botones de acción', async function () {
    ...
    // - NO logueado: Comenzar Gratis / Iniciar Sesión
    // - Logueado (bypass o sesión real): Ver Procesos / Crear Proceso
  ...
  // ACTIVIDAD LIST
  //
  it('Actividades: debería mostrar título, botón "Nueva Actividad" y filtros de búsqueda', async function () {
    // Ajusta "1" por un procesoId que tengas en tus pruebas (si aplica)
    await driver.get('http://localhost:4200/procesos/1/actividades');

    // Título
    const titulo = await driver.wait(
      until.elementLocated(
        By.xpath("//h2[contains(., 'Actividades del Proceso')]")
      ),
      10000
    );
  ...
  it('Actividad Form: el botón "Crear/Actualizar Actividad" debe estar deshabilitado si el formulario es inválido y habilitarse con datos válidos', async function () {
    // Ajusta "1" por un procesoId válido en tu entorno de pruebas
    ...
    // Botón submit: tiene texto "Crear Actividad" o "Actualizar Actividad"
    ...
    // Tomamos la primera opción válida distinta de "Selecciona un tipo"
    ...
    // Opcional: NO hacemos submit para no depender de backend
  ...
  // EMPRESA LIST
  //
  it('Empresa: debería mostrar título y algún estado (card, mensaje de vacío o error)', async function () {
  ...
  it('Empresa Form: si hay botón Editar, debe llevar al formulario con los campos principales visibles', async function () {
    ...
    // Botón "Editar" en la card, si el usuario (o bypass) es admin
